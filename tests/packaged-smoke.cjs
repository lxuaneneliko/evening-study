...
const files = ['electron/locked-window.cjs', 'renderer/locked-bootstrap.js', 'electron/spotify.cjs', 'renderer/locked.html', 'renderer/locked.js', 'electron/main.cjs', 'electron/preload.cjs', 'shared/core.js', 'shared/default-schedule.json', 'renderer/widget.html', 'renderer/widget.js', 'renderer/manager.html', 'renderer/manager.js', 'renderer/styles.css', 'renderer/ui.js'];
...
(async () => {
  ...
  try {
    ...
    assert.equal(result.packaged, true); assert.equal(result.version, require('../package.json').version);
    ...
    console.log(`PASS packaged Windows executable launches; ${files.length} source files match; version, 35 entries, 8 resize handles verified`);
