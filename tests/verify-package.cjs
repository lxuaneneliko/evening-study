...
const report = { version, verifiedAt: new Date().toISOString(), verifiedFiles: files, archiveSha256: hash(fs.readFileSync(archive)), portableSha256: hash(fs.readFileSync(portable)), sourceFilesMatch: true, runtimeTest: 'not-run-application-control-signing-required' };
